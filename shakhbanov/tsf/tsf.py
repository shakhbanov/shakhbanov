...
class LightGBMModel:
    ...
    def objective(self, trial, X, y, valid_categorical_features):
        params = {
            'objective': 'regression',
            'metric': 'mse',
            'learning_rate': trial.suggest_loguniform('learning_rate', 1e-4, 0.05),  # Уменьшение верхней границы для более плавного обучения
            'num_leaves': trial.suggest_int('num_leaves', 31, 80),  # Уменьшение диапазона для стабильности модели
            'min_data_in_leaf': trial.suggest_int('min_data_in_leaf', 30, 60),  # Увеличение минимального количества данных в листе
            'feature_fraction': trial.suggest_uniform('feature_fraction', 0.8, 1.0),  # Увеличение нижней границы для использования большего числа признаков
            'bagging_fraction': trial.suggest_uniform('bagging_fraction', 0.8, 1.0),  # Увеличение нижней границы для большего охвата данных
            'bagging_freq': trial.suggest_int('bagging_freq', 1, 5),
            'min_gain_to_split': trial.suggest_loguniform('min_gain_to_split', 1e-3, 1e-1),  # Добавление параметра для контроля разбиений
            'max_depth': trial.suggest_int('max_depth', 5, 15),  # Ограничение глубины дерева для предотвращения переобучения
            'num_threads': n_jobs,
            'verbose': -1
        }
    ...
    def fit(self, X, y):
        categorical_features = ['preciptype', 'city', 'format', 'working_hours']
...
class ProphetsEnsemble:
    ...
    def objective(self, trial, data, level):
        params = {
            ...
            'holidays_prior_scale': trial.suggest_loguniform('holidays_prior_scale', 10, 20),
        ...
        for regressor in ['tempmax', 'temp', 'tempmin', 'precip', 'humidity', 'windspeed', 'cloudcover', 'solarenergy', 'uvindex', 'area', 'usd_lag_2m', 'usd_avg_q']:
            if regressor in resampled.columns:
                fb.add_regressor(regressor)

        for regressor in ['format', 'preciptype', 'cluster', 'working_hours']:
            if regressor in resampled.columns:
                fb.add_regressor(regressor, standardize=False)
...
class ForecastAdjustment:
    ...
    def get_last_week_data(self, rest_id, target):
        ...
        forecast_data = self.forecast[(self.forecast['rest_id'] == rest_id) & (self.forecast['ds'].isin(last_week_dates))] if self.forecast is not None else None
