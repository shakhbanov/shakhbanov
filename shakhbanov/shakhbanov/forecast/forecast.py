...
class ProphetsEnsemble:
    ...
    def __init__(self, freq: str, levels: list, agg_fn: list, holidays_getter: HolidayBase = None, prophet_params: dict = None):
        self.freq = freq
...
class BurgerKing:
    ...
    Attributes:
        df (pd.DataFrame): DataFrame containing the restaurant data.
        tqdm_enabled (bool): Flag to enable or disable tqdm progress bars.
    """

    def __init__(self, df: pd.DataFrame, freq: str = 'D', periods: int = 180, target: list = None,
                 levels: list = None, agg_fn: list = None, n_splits: int = 5, n_estimators: int = 350,
                 early_stopping_rounds: int = 150, lgb_params: dict = None, prophet_params: dict = None,
                 holidays_getter: HolidayBase = None, tqdm_enabled: bool = True):
        self.df = df
        self.freq = freq
        self.periods = periods
        self.target = target if target else ['check_qnty', 'avg']
        self.levels = levels if levels else ['W', 'M']
        self.agg_fn = agg_fn if agg_fn else ['mean', 'median']
        self.n_splits = n_splits
        self.n_estimators = n_estimators
        self.early_stopping_rounds = early_stopping_rounds
        ...
        tqdm_disabled = not tqdm_enabled
    ...
    def process_restaurant_variable(self, rest_id: int, column: str) -> list:
        ...
        """
        results = []
        df_rest = self.df[self.df['rest_id'] == rest_id].copy()
        ...
        pe = ProphetsEnsemble(freq=self.freq, levels=self.levels, agg_fn=self.agg_fn,
    ...
    def run(self) -> pd.DataFrame:
        ...
        """
        restaurants = self.df['rest_id'].unique()
        target = self.target
