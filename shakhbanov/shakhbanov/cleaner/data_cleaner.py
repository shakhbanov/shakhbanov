...
class DataCleaner:
    ...
    def process_series(self, series_data: pd.DataFrame, column: str) -> pd.DataFrame:
        """
        ...
        series_data.loc[:, 'moving_average'] = series_data[column].rolling(window=self.window, min_periods=1, center=True).mean()
        series_data.loc[:, 'std_dev'] = series_data[column].rolling(window=self.window, min_periods=1, center=True).std()
        series_data.loc[:, 'lower'] = series_data['moving_average'] - self.std * series_data['std_dev']
        series_data.loc[:, 'upper'] = series_data['moving_average'] + self.std * series_data['std_dev']
        ...
        for day in range(7):
            ...
            for idx in day_data.index:
                if day_data.loc[idx, column] < day_data.loc[idx, 'lower'] or day_data.loc[idx, column] > day_data.loc[idx, 'upper']:
                    reference_date = day_data.loc[idx, 'ds']
                    mean_value = self.calculate_week_average(day_data, column, reference_date, day_data.loc[idx, 'lower'], day_data.loc[idx, 'upper'])
                    series_data.loc[idx, column] = mean_value

        return series_data

    def clean_data(self, data: pd.DataFrame, column: str, series_id: str = 'series_id') -> pd.DataFrame:
        """
        ...
        if self.parallel:
            results = Parallel(n_jobs=-1)(delayed(self.process_series)(restored_data[restored_data[series_id] == series], column) for series in unique_series_ids)
            restored_data = pd.concat(results)
        else:
            restored_data = pd.concat([self.process_series(restored_data[restored_data[series_id] == series], column) for series in unique_series_ids])
        ...
        # Visualization
        if self.plot:
            self.plot_data(data, restored_data, column, series_id)

        return restored_data

    def plot_data(self, original_data: pd.DataFrame, cleaned_data: pd.DataFrame, column: str, series_id: str):
        """
        ...
        :param column: Column being cleaned
        :param series_id: Column name that identifies different time series
        """
        ...
        for series in unique_series_ids:
            ...
            plt.xlabel('Date')
            plt.ylabel(column)
            plt.title(f'{series_id.capitalize()} {series}')
            plt.legend()
            plt.show()
